...
var ASM_CONSTS = {
  8430056: ($0) => { console.log('[DEBUG WASM] klh10_set_ring_buffer_offset called with offset:', $0.toString(16)); },  
 8430155: ($0) => { console.log('[DEBUG WASM] Setting up ring buffers in WASM memory at offset:', $0.toString(16)); },  
 8430255: ($0, $1, $2) => { console.log('[DEBUG WASM] Initializing ONLY size field, preserving JavaScript write_pos/read_pos'); console.log('[DEBUG WASM] BEFORE: write_pos=' + $0 + ', read_pos=' + $1 + ', size=' + $2); },  
 8430450: ($0, $1, $2) => { console.log('[DEBUG WASM] AFTER: write_pos=' + $0 + ', read_pos=' + $1 + ', size=' + $2); },  
 8430544: ($0) => { console.log('[DEBUG WASM] Ring buffers initialized in WASM memory. Input at offset:', ($0).toString(16)); },  
 8430654: ($0) => { if (typeof postMessage !== 'undefined') { postMessage({ type: 'mode_change', data: $0 === 0 ? 'run' : 'command' }); } },  
 8430776: ($0, $1) => { var sharedMemory = Module.wasmMemory; if (!sharedMemory || !sharedMemory.buffer) { console.error('[DEBUG WASM] No shared WASM memory available in os_ttyout'); return 0; } var buffer = new Uint8Array(sharedMemory.buffer); var view = new DataView(sharedMemory.buffer); var baseOffset = $0; var char = $1; var size = 4096; var write_pos = view.getUint32(baseOffset, true); var read_pos = view.getUint32(baseOffset + 4, true); var next_write_pos = (write_pos + 1) % size; if (next_write_pos === read_pos) { return 0; } buffer[baseOffset + 16 + write_pos] = char; view.setUint32(baseOffset, next_write_pos, true); return 1; },  
 8431399: ($0) => { console.log('[DEBUG WASM] ============================================'); console.log('[DEBUG WASM] os_ttycmline() CALLED! buffer size=' + $0); console.log('[DEBUG WASM] ============================================'); },  
 8431621: () => { console.log('[DEBUG WASM] os_ttycmline: Starting to wait for input...'); },  
 8431698: ($0) => { console.log('[DEBUG WASM] os_ttycmline: Found line ending, returning line: "' + UTF8ToString($0) + '"'); },  
 8431807: ($0) => { console.log('[DEBUG WASM] ring_buffer_has_complete_line: found line ending at pos ' + $0); },  
